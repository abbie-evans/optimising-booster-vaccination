...
def calculate_susc(t, boosted, boost_time, recovered_time, entry_time, susceptibility, hosp_protection):
    # Calculate susceptibility based on the population's vaccination status and time since last boost
    
    # Condition 1: Not boosted, not recovered
    ...
    boosted_not_recovered = (boosted) & (np.isnan(recovered_time))
    ...
    boosted_recovered = (boosted) & (~np.isnan(recovered_time))
...
def calculate_force_of_infection(init_susc, age_group, status):
    ...
    beta_new = 1.5/R0*parameters['beta'] # Set Re value
...
def boost_people(current_time, status, boosted, boost_time, to_boost_time, dead, pop):
...
def boost_people_second(current_time, status, boosted_second, boost_time, to_boost_time, dead, pop):
    to_boost = ((status[pop] == 0) | (status[pop] == 1)) & (~boosted_second[pop]) & (~dead[pop]) & (to_boost_time[pop] < current_time)
    # Select individuals to boost based on the number of boosts available
    num_boost = parameters["num_boost"]
    # Find indices of pop where to_boost is True
    to_boost_indices = pop[to_boost]
    ...
    selected_indices = to_boost_indices[:num_boost]
    boost_time[selected_indices] = current_time
    boosted_second[selected_indices] = True
...
def run(delay, second_delay, sim_num=0):
    base_seed = 42
    ...
    boosted_second = np.zeros(100000, dtype=bool)
    boost_time = np.full(100000, np.nan)
    to_boost_time = np.full(100000, delay)
    to_boost_second_time = np.full(100000, np.nan)
    ...
    calculate_susc(0, boosted, boost_time, recovered_time, entry_time, susceptibility, hosp_protection)
    ...
    all_first_dose = False
    all_second_dose = False
    for t in range(365):
        if t == 0:
            calculate_susc(t, boosted, boost_time, recovered_time, entry_time, susceptibility, hosp_protection) # Initialise susceptibility
        ...
        if t > 0:
            calculate_susc(t, boosted, boost_time, recovered_time, entry_time, susceptibility, hosp_protection)
        ...
        # check if everyone in pop has been boosted
        if np.all(boosted[pop] | dead[pop]) and not all_first_dose:
            all_first_dose = True
            # if so, can now begin second booster doses
            to_boost_second_time = np.full(100000, second_delay)
        boost_people_second(t, status, boosted_second, boost_time, to_boost_second_time, dead, pop)

        if np.all(boosted_second[pop] | dead[pop]) and not all_second_dose:
            all_second_dose = True

    return new_infect, new_death, age_death, new_hosp

os.makedirs(f'data/two_doses', exist_ok=True)
...
for delay in np.linspace(0, 181, 180, dtype=int):
    ...
    for second_delay in range(delay+40, 365):
        print(f"Running simulation with second dose delay {second_delay}...")
    # Run the simulation with the specified delay
        all_results = Parallel(n_jobs=-1)(delayed(run)(delay, second_delay, _) for _ in tqdm(range(100)))
    ...
    inf_df.to_csv(f'data/two_doses/infections_{delay}.csv', header=None, index=False)
    df = pd.DataFrame(death)
    df.to_csv(f'data/two_doses/deaths_{delay}.csv', header=None, index=False)
    age_deaths_df = pd.DataFrame(avg_age_deaths)
    age_deaths_df.to_csv(f'data/two_doses/age_deaths_{delay}.csv', header=None, index=False)
    hosp_df = pd.DataFrame(hosp)
    hosp_df.to_csv(f'data/two_doses/hospitalisations_{delay}.csv', header=None, index=False)
print(np.sum(death, axis=1))
