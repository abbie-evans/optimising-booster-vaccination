...
for delay in [400, 0, 109]: # Here 400 represents no vaccination, 0 represents no delay and 109 represents a 109 day delay
    ...
    all_results = Parallel(n_jobs=-1)(delayed(run)(delay, _) for _ in tqdm(range(1000)))
    ...
    inf_df.to_csv(f'data/infections_{delay}.csv', header=None, index=False)
    df = pd.DataFrame(death)
    df.to_csv(f'data/deaths_{delay}.csv', header=None, index=False)
    age_deaths_df = pd.DataFrame(avg_age_deaths)
    age_deaths_df.to_csv(f'data/age_deaths_{delay}.csv', header=None, index=False)
    df = pd.DataFrame(hosp)
    df.to_csv(f'data/hospitalisations_{delay}.csv', header=None, index=False)
