...
def run(delay, sim_num=0):
    ...
    exposed_indices = np.random.choice(np.arange(100000), size=200, replace=False)
    status[exposed_indices] = 1 # STATUS_EXPOSED
    latent_period[exposed_indices] = np.random.choice(range(1, 40), size=200, p=l_period)
...
os.makedirs(f'data/re1.5/200', exist_ok=True)
