...
os.makedirs(f'data/s4/re1.5/1.5', exist_ok=True)
...
# Save the results to a CSV file
    df = pd.DataFrame(inf)
    df.to_csv(f'data/s4/re1.5/1.5/infections_{delay}.csv', header=None, index=False)
    df = pd.DataFrame(death)
    df.to_csv(f'data/s4/re1.5/1.5/deaths_{delay}.csv', header=None, index=False)
    age_deaths_df = pd.DataFrame(avg_age_deaths)
    age_deaths_df.to_csv(f'data/s4/re1.5/1.5/age_deaths_{delay}.csv', header=None, index=False)
    df = pd.DataFrame(hosp)
    df.to_csv(f'data/s4/re1.5/1.5/hospitalisations_{delay}.csv', header=None, index=False)
