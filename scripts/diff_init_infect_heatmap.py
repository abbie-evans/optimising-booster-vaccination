...
for multiplier in [1, 1.25, 1.5, 1.75, 2, 2.25, 2.5]:
    deaths = pd.read_csv(f'data/re1.5/50/deaths_{multiplier}.csv', header=None, dtype=float)
...
plt.tight_layout()
plt.savefig("figures/deaths_init_50.svg", bbox_inches="tight")
plt.show()

delay_deaths = []
for multiplier in [1, 1.25, 1.5, 1.75, 2, 2.25, 2.5]:
    deaths = pd.read_csv(f'data/re1.5/200/deaths_{multiplier}.csv', header=None, dtype=float)
    delay_deaths.append(list(deaths.sum(axis=1)))
...
plt.xlabel(r'Vaccine efficacy ($\sigma$)', labelpad=10, fontsize=20)
plt.tight_layout()
plt.savefig("figures/deaths_init_200.svg", bbox_inches="tight")
plt.show()
