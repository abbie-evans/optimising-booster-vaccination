...
def run(delay, sim_num=0):
    ...
    exposed_indices = np.random.choice(np.arange(100000), size=50, replace=False)
    status[exposed_indices] = 1 # STATUS_EXPOSED
    latent_period[exposed_indices] = np.random.choice(range(1, 40), size=50, p=l_period)
...
os.makedirs(f'data/re1.5/50', exist_ok=True)
...
inf_df.to_csv(f'data/re1.5/50/infections_{multiplier}.csv', header=None, index=False)
df = pd.DataFrame(death)
df.to_csv(f'data/re1.5/50/deaths_{multiplier}.csv', header=None, index=False)
df = pd.DataFrame(hosp)
df.to_csv(f'data/re1.5/50/hospitalisations_{multiplier}.csv', header=None, index=False)
age_deaths_df = pd.DataFrame(avg_age_deaths)
age_deaths_df.to_csv(f'data/re1.5/50/age_deaths_{multiplier}.csv', header=None, index=False)
