...
def calculate_force_of_infection(init_susc, age_group, status, re):
    ...
    beta_new = re/R0*parameters['beta'] # Set Re value
...
def run(delay, re, sim_num=0):
    ...
    for t in range(365):
        ...
        force_of_infection = calculate_force_of_infection(init_susc, age_group, status, re)
        if t > 0:
...
os.makedirs(f'data/optim_timing/{multiplier}', exist_ok=True)
...
for re in np.arange(1.5, 3.1, 0.1):
    for delay in np.linspace(0, 181, 180, dtype=int):
        ...
        all_results = Parallel(n_jobs=-1)(delayed(run)(delay, re, _) for _ in tqdm(range(1000)))
    ...
    inf_df.to_csv(f'data/optim_timing/{multiplier}/infections_{re}.csv', header=None, index=False)
    df = pd.DataFrame(death)
    df.to_csv(f'data/optim_timing/{multiplier}/deaths_{re}.csv', header=None, index=False)
    age_deaths_df = pd.DataFrame(avg_age_deaths)
    age_deaths_df.to_csv(f'data/optim_timing/{multiplier}/age_deaths_{re}.csv', header=None, index=False)
    df = pd.DataFrame(hosp)
    df.to_csv(f'data/optim_timing/{multiplier}/hospitalisations_{re}.csv', header=None, index=False)
